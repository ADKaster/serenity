...
static WebIDL::ExceptionOr<::Crypto::PK::RSAPrivateKey<>> parse_jwk_rsa_private_key(JS::Realm& realm, Bindings::JsonWebKey const& jwk)
{
    ...
    if (jwk.p.has_value()) {
        // FIXME: Pass the other parameters to the constructor if provided (once they are added to the ctor)
        // We know that if any of them are provided, all of them must be
    }
