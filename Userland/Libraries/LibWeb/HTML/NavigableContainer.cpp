...
void NavigableContainer::navigate_an_iframe_or_frame(AK::URL url)
{
    ...
    if (!m_content_navigable->active_document()->is_completely_loaded()) {
    ...
    MUST(m_content_navigable->navigate(url, document(), Empty {}, nullptr, false, history_handling));
}
