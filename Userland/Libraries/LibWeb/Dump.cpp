...
static ErrorOr<void> dump_session_history_entry(StringBuilder& builder, HTML::SessionHistoryEntry const& session_history_entry, int indent_levels)
{
    ...
    TRY(builder.try_appendff("step=({}) url=({}) document-is-nullptr=({}) is-active=({})\n", session_history_entry.step.get<int>(), session_history_entry.url, document.ptr() == nullptr, document && document->is_active()));
