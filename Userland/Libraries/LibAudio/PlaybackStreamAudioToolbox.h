...
class PlaybackStreamAudioToolbox final : public PlaybackStream {
public:
...
private:
    struct AudioToolboxState;
    friend AudioToolboxState;

    explicit PlaybackStreamAudioToolbox(RefPtr<AudioToolboxState>);
    ~PlaybackStreamAudioToolbox();

    RefPtr<AudioToolboxState> m_impl;
};
